...
## What I built
`llm_practice.py` calls the chat completions API and sweeps temperature — `[0, 0.3, 0.7, 1.0]` — on one fixed prompt, printing the output at each setting for direct comparison. It's built around a small wrapper class over the chat completions endpoint with retry/backoff and clean parameter passthrough (temperature, logprobs, etc.) — see `azure_llm_wrapper.py` for the full implementation.
